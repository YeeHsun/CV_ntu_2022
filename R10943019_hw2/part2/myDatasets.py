...
class cifar10_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_root = self.prefix
        img_path = os.path.join(img_root, img_name)
        img = Image.open(os.path.join(img_path)).convert('RGB')

        if self.transform is None:
            #testing dont need to apply transformation
            return_image = transforms.ToTensor()(img)
        else: 
            return_image = self.transform(img)
        if self.labels is None:
            return return_image
        else:
            return_label=torch.tensor(self.labels[idx]).to(torch.int64)
