...
def train(model, train_loader, val_loader, num_epoch, log_path, save_path, device, criterion, scheduler, optimizer):
    ...
    train_acc_curve = plt.plot(x, overall_acc, 'b')
    val_acc_curve = plt.plot(x, overall_val_acc, 'g')
    plt.legend(["training", "validation"], loc='best')
    plt.savefig(os.path.join(save_path, 'acc_{}.png'.format(best_acc)))
    plt.close()
    ...
    plt.xlabel('epoch', fontsize=10)
    plt.ylabel('acc', fontsize=10)
    train_loss_curve = plt.plot(x, overall_loss, 'b')
    val_loss_curve = plt.plot(x, overall_val_loss, 'g')
    plt.legend(["training", "validation"], loc='best')
    plt.savefig(os.path.join(save_path, 'loss_{}.png'.format(best_acc)))
    plt.close()
