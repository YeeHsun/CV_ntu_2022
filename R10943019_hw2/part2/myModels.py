...
class myResnet(nn.Module):
    def __init__(self, in_channels=3, num_out=10):
        ...
        self.residual1 = residual_block(in_channels=64)
        self.conv1 = nn.Sequential(nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, padding=1),
                                   nn.MaxPool2d(kernel_size=2, stride=2),
                                   nn.BatchNorm2d(64),
                                   nn.Dropout(p=0.2),
        ...
        self.residual2 = residual_block(in_channels=128)
        self.conv2 = nn.Sequential(nn.Conv2d(in_channels=128, out_channels=128, kernel_size=3, padding=1),
                                   nn.MaxPool2d(kernel_size=2, stride=2),
                                   nn.BatchNorm2d(128),
                                   nn.Dropout(p=0.2),
                                   nn.ReLU(inplace=True),
        ...
        self.residual3 = residual_block(in_channels = 256)
    ...
    def forward(self,x):
        ...
        x = self.residual2(x)
        x = self.conv2(x)
        x = self.residual3(x) # 16*256*5*5
        #print(x.shape)
        x = x.flatten(1,-1) # 16*1024
        x = self.fc1(x)
        x = self.fc2(x)
        x = self.fc3(x)
        return x
